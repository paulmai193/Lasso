...
class GetConfirmJobSerializer extends JsonSerializer<GetOrderResponse> {
	...
	@Override
	public void serialize(GetOrderResponse __value, JsonGenerator __gen, SerializerProvider __serializers)
		...
		_styles.forEach(__style -> {
			try {
				...
				__gen.writeObjectFieldStart("images");
				GetConfirmJobSerializer.this.serializeImage(__gen, __value.getPrefixStyle(), __style.getImage());
				__gen.writeEndObject();
				__gen.writeEndObject();
		...
		this.serializeAccounts(__gen, _designersJobs, __value.getPrefixAvatar(), __value.getPrefixPortfolio());
		__gen.writeEndArray();

		__gen.writeArrayFieldStart("confirm");
		this.serializeAccounts(__gen, _confirm, __value.getPrefixAvatar(), __value.getPrefixPortfolio());
		__gen.writeEndArray();

		__gen.writeArrayFieldStart("counter_offer");
		this.serializeCounterAccounts(__gen, _counterOffer, __value.getPrefixAvatar(), __value.getPrefixPortfolio());
		__gen.writeEndArray();
		__gen.writeEndObject();
		__gen.writeEndObject();
	...
	private void serializeAccounts(JsonGenerator __gen, List<Object[]> __unConfirm, String __prefixAvatar,
		...
		__unConfirm.forEach(__accounts -> {
			try {
				...
				__gen.writeNumberField("designer_reward", _account.getRewards() == 0 ? 1 : _account.getRewards());
				__gen.writeObjectFieldStart("designer_avatar");
				GetConfirmJobSerializer.this.serializeImage(__gen, __prefixAvatar, _account.getImage());
				__gen.writeEndObject();
				__gen.writeObjectFieldStart("portfolio_image");
				String _portfolioImage = _portfolio.getImage().split(",")[0];
				GetConfirmJobSerializer.this.serializeImage(__gen, __prefixPortfolio, _portfolioImage);
				__gen.writeEndObject();
				__gen.writeEndObject();
			} catch (IOException _ex) {
				Logger.getLogger(this.getClass()).warn("Unwanted error", _ex);
	...
	private void serializeCounterAccounts(JsonGenerator __gen, List<Object[]> __counterAccounts, String __prefixAvatar,
			String __prefixPortfolio) {
...
public class GetOrderResponse extends BaseResponse {
	 ...
	 * @param __prefixJob
	 *            the prefix job
	 * @param __prefixPortfolio
	 *            the prefix portfolio
	 */
	public GetOrderResponse(Object[] __data, String __prefixAvatar, String __prefixStyle, String __prefixType,
			String __prefixCategory, String __prefixJob, String __prefixPortfolio) {
		super();
		this.data = __data;
