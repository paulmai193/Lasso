...
public class ImplDesignerManagement extends ImplProjectManagement implements DesignerManagement {
	...
	public void createPortfolio(Account __desiger, CreatePortfolioRequest __createPortfolioRequest)
		...
		try {
			...
			List<Portfolio> _portfolios = this.portfolioDAO
			        .getPortfolioOfAccountByStyle(_style.getId(), __desiger.getId());
	...
	public void editPortfolio(Account __desiger, EditPortfolioRequest __editPortfolioRequest)
		...
		try {
			Category _category = this.categoryDAO
			        .getCategoryById(__editPortfolioRequest.getIdCategory());
			Style _style = this.styleDAO.getById(__editPortfolioRequest.getIdStyle());
			Portfolio _portfolio = this.portfolioDAO.getPortfolioOfAccount(__desiger,
			        __editPortfolioRequest.getId());
			if (_category == null || _category.getDeleted().equals((byte) 1)) {
				throw new NotFoundException("Category not found");
			}
			if (_style == null || _style.getDeleted().equals((byte) 1)) {
				throw new NotFoundException("Style not found");
			}
			if (_portfolio == null) {
			...
			List<Portfolio> _portfolios = this.portfolioDAO
			        .getPortfolioOfAccountByStyle(_style.getId(), __desiger.getId());
			if (_portfolio.getStyleId().intValue() != _style.getId().intValue()
			        && _portfolios.size() > 0) {
				throw new ForbiddenException(
				        "This style was set in other portfolio. Please choose another.");
			}
			...
			this.portfolioDAO.updatePortfolio(_portfolio);

			// Remove all old portfolio type
			this.portfolioTypeDAO.removeByPortfolioId(_portfolio.getId());
