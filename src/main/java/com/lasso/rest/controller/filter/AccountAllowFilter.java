...
public class AccountAllowFilter implements ContainerRequestFilter {
	 ...
	 */
	private void checkPermissions(List<String> __allowedStatus, Byte __statusMustCheck, List<String> __allowedRoles,
			Byte __roleMustCheck) throws AuthenticateException {
		if (__allowedStatus.contains(__statusMustCheck.toString())) {
			if (__allowedRoles.contains(__roleMustCheck.toString())) {
				return;
			} else {
				throw new AuthenticateException("User's role not allow to access", Status.FORBIDDEN);
			}
		} else {
			throw new AuthenticateException("User's current status not allow to access", Status.FORBIDDEN);
		}
	}
	...
	public void filter(ContainerRequestContext __requestContext) throws IOException {
		...
		try {
			Account _authenticatedAccount = (Account) __requestContext.getSecurityContext().getUserPrincipal();
